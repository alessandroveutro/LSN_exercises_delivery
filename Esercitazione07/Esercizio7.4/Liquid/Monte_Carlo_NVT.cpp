...
{ 
  ...
  Config(); //Read configuration from file config.0
  Equilibrium();
  int nconf = 1;
...
void Equilibrium(){
  ...
  Epot.open("eq_epot.out");
  Pres.open("eq_pres.out");
  ...
  for(int j = 0; j < 5; j++){
      ...
      for(int istep = 1; istep <= nstep_eq; ++istep){
         ...
         double u = walker[iv];
         double p = rho*temp + walker[iw]/vol;
         Epot << u/(double)npart << endl;
         Pres << p << endl;
      }

      ConfFinal(); //Scrivo su file la configurazione a fine ciclo
      OverWrite(); //Sovrascrivo le coordinate iniziali con quelle finali
      nstart ++; //accumulo il numero di cicli
...
void WriteIst()
{
  ...
  Epot.open("ist_epot.out",ios::app);
  Pres.open("ist_pres.out",ios::app);

  double u = walker[iv];
  double p = rho*temp + walker[iw]/vol;

  Epot << u/(double)npart << endl;
  Pres << p/(double)npart << endl;
