...
double external_potential(double val)
{
	...
	return pow(val,4.0) - 2.5*pow(val,2.0);
}
...
double external_potential_prime(double val)
{
	...
	return 4.0*pow(val,3.0) - 5.0*val;
}
...
double variationalWaveFunction(double v)
{
	...
	//double a1 = -(v - mu)*(v - mu)/(2.0*sigma*sigma);
	//double a2 = -(v + mu)*(v + mu)/(2.0*sigma*sigma);

	//return exp(a1) + exp(a2);
}
...
double variationalWaveFunction_second(double v)
{
	...
	//double a1 = -(v - mu)*(v - mu)/(2.0*sigma*sigma);
	//double a2 = -(v + mu)*(v + mu)/(2.0*sigma*sigma);
	//double b1 = (v - mu)*(v - mu)/pow(sigma,4.0);
	//double b2 = (v + mu)*(v + mu)/pow(sigma,4.0);

	//return b1*exp(a1) + b2*exp(a2) - (1.0/(sigma*sigma))*variationalWaveFunction(v);
}
